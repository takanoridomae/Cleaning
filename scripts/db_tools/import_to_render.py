#!/usr/bin/env python
"""
JSONファイルからRender環境のデータベースにデータをインポートする
"""
import sys
import os
import json
...
def import_data_to_render(json_file):
    """JSONファイルからデータをRender環境にインポート"""
    app = create_app()
    ...
    with app.app_context():
        try:
            ...
            print(f"Render環境へのデータインポート開始: {json_file}")
            ...
            if "photos" in import_data:
                ...
                print("※画像ファイルはRender環境への手動アップロードが必要です")
            ...
            print(f"\nRender環境へのデータインポート完了!")
            print("注意: 写真ファイルは別途手動でアップロードが必要です")
            return True

        except Exception as e:
            print(f"Render環境インポートエラー: {e}")
            db.session.rollback()
            return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法: python import_to_render.py <json_file>")
        print("例: python import_to_render.py db_export_20250624_123456.json")
    else:
        import_data_to_render(sys.argv[1])
