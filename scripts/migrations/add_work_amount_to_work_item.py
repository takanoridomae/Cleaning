#!/usr/bin/env python3
"""
作業項目マスター (work_items) テーブルに作業金額フィールドを追加するマイグレーション
"""
...
def add_work_amount_column():
    """work_itemsテーブルに作業金額フィールドを追加"""

    app = create_app()
    with app.app_context():
        try:
            print("=== work_itemsテーブルに作業金額フィールドを追加 ===")

            # 現在のテーブル構造を表示
            with db.engine.connect() as connection:
                result = connection.execute(db.text("PRAGMA table_info(work_items)"))
                print("現在のwork_itemsテーブル構造:")
                columns = []
                for row in result:
                ...
                connection.execute(
                    db.text(
                        "ALTER TABLE work_items ADD COLUMN work_amount INTEGER DEFAULT 0"
                    )
                )
